...
- `emitUTXOCreated(ctx, utxo, blockHeight, classification) error` — Internal helper that null-safely builds a `ledger.UTXOCreatedMsg` from a wallet `Utxo` and Tells it to `ledgerSink`. Negative block heights clamp to `0` rather than wrapping under a direct `uint32` cast; nil `utxo` and `fn.None` sink are silent no-ops. A refused enqueue is **returned**, not logged: `processUtxo` runs this inside `InsertBoardingIntents`'s transaction so the deposit leg and the intent row commit together.
...
- **Sends**:
  ...
  - → `ledger` actor (via `ledger.Sink` Tell, when `fn.Some`): `UTXOCreatedMsg` on every processed confirmed wallet UTXO, tagged `ClassificationDeposit`. `handleUTXOCreated` expands this into both a `wallet_utxo_log` audit row AND a double-entry deposit leg (debit `wallet_balance`, credit `opening_balance`). `processUtxo` enqueues it from inside `BoardingStore.InsertBoardingIntents`'s transaction, so a refused enqueue rolls the intent back and leaves the UTXO out of `seenUtxos` for the next tip tick — the deposit leg is the only producer of that UTXO's `wallet_balance` debit, while the later boarding leg credits `wallet_balance` unconditionally. Finalized boarding sweeps emit a single `BoardingSweepConfirmedMsg` (txid, chain cost, per-input list, destination) that the ledger expands into the fee, per-input, and destination clearing legs inside one Commit, so `wallet_clearing` is updated atomically. `commitFinalizedSweep` enqueues that message from inside `BoardingSweepStore.FinalizeBoardingSweepInputs`'s transaction, so a refused enqueue rolls the input reconcile back and the next start re-drives the sweep instead of overstating `wallet_balance`.
...
- `WithEagerRoundJoin(true)` opts the wallet into "drive round-joining without a second RPC" semantics. Two sites change: `handleBlockEpoch` inline-calls `handleBoard` after at least one new boarding UTXO confirms in the block (one `TriggerBoardMsg` per block, not per UTXO), and `handleLeaveVTXOs` forwards its `RegisterIntentMsg` with `TriggerRegistration=true` so the leave moves the round FSM out of `PendingRoundAssembly` immediately. Default off preserves the operator-driven batched semantics that `wavecli` and server hosts rely on; `sdk/wavewalletdk` flips it on via `waved.Config.EagerRoundJoin` for wallet-shaped SDK hosts.
- The deposit leg and the boarding intent are one durable fact. `seenUtxos`
  permanently suppresses re-detection of a processed outpoint, so a deposit
  leg dropped after the intent committed could never be re-emitted while the
  later boarding leg still credits `wallet_balance`. `processUtxo` therefore
  enqueues `UTXOCreatedMsg` inside `InsertBoardingIntents`'s transaction and
  only adds to `seenUtxos` once that transaction commits.
- Leave destinations are booked as foreign unless the `owned_wallet_scripts` registry says the daemon minted them. `destinationIsOwnWallet` fails closed on every path — no registry wired, an empty script, a lookup error, an unknown script — because a leave must not fail over an accounting flag and overstating ownership would understate what the client paid out.
- The boarding-limit change leave stays unflagged on purpose: it pays a boarding script whose later `UTXOCreatedMsg` deposit leg already books the return. `boardingChangeLeave` verifies the destination against the boarding-address registry and fails the board rather than silently losing that deposit leg if a refactor changes the destination.
- A boarding deposit reports facts, it does not judge them. `processUtxo` attaches the funding transaction's previous outpoints to the deposit's `UTXOCreatedMsg` and lets the ledger actor decide which of them it already credited; the wallet reads no audit rows and depends on no message ordering.
- The change credit for a partial spend of those proceeds is enqueued inside the same `InsertBoardingIntents` transaction as the deposit leg. A crash between them would leave the client understated with no producer left to correct it, since `seenUtxos` permanently suppresses re-detection.
