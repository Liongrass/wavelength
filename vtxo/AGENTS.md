...
- `CheckForfeitAdmission(desc *Descriptor) error` — advisory pre-check
  answering whether a VTXO may be committed to a *new* cooperative forfeit
  (leave, refresh, or in-round send). `Live` and `Expired` are admissible —
  expired coins are recoverable only by forfeiting them into an ordinary
  round, so refusing them would strand the value permanently. Everything else
  returns a sentinel naming what holds the coin: `ErrForfeitInFlight`
  (`PendingForfeit`/`Forfeiting`, with the round ID once one is stamped),
  `ErrVTXOLiquidityLocked` (`Spending`), `ErrExitInFlight`
  (`UnilateralExit`), or `ErrVTXOTerminal` (terminal, or a nil descriptor).
  Callers: `waved.admitLeaveTargets` and `waved.exitPlanRoundCommitment`.
...
- **Startup sweep of orphaned forfeits.** `Start` calls `releaseOrphanedForfeits` after actor recovery. Any VTXO still in `VTXOStatusPendingForfeit` is provably orphaned and returns to `LiveState`. A `Forfeiting` VTXO can also be stranded before signature handoff because actors persist their signed transaction while the round is still collecting peers. The manager releases it only when `forfeit_round_id` is present and the round store explicitly reports no durable checkpoint; a missing binding, lookup error, or existing round row fails closed and retains `Forfeiting`.
- **Confirmed-forfeit startup reconcile.** The serialized `ReconcileExpiry` manager turn runs after the round actor starts. It lists recovered `Forfeiting` descriptors and re-drives `ForfeitConfirmedEvent` only when the joined `Settlement` has `Height > 0`, closing a crash between round finalization and the original best-effort confirmation Tell. A missing settlement, height zero, lookup error, missing actor, or actor error fails closed in `Forfeiting`.
...
- `Manager.handleForceUnroll` uses `Ask` (not `Tell`) so FSM errors and self-loop no-ops surface as structured `ForceUnrollResponse{Accepted, Reason}` instead of a uniform `Accepted:true` that masks work that was never scheduled. Every self-loop `ForceUnrollEvent` case must have a matching branch here; a new one that does not is reported as an accepted exit.
- **A second `PendingForfeitEvent` is refused, in `PendingForfeitState` as well as in `ForfeitingState`.** `Manager.handleReserveForfeit` validates only that an actor exists and that the outpoint is not spend-reserved, then trusts the FSM, so the FSM is the only place a duplicate claim can be caught. `PendingForfeitState` used to self-loop on it as a harmless duplicate, which meant a leave whose advisory `CheckForfeitAdmission` pre-check passed while the coin was live, and which then raced a concurrent claim, was accepted and reported as queued while the first claim's destination is what actually gets paid. Both refusals name the outpoint and state (and the round id, once `ForfeitingState` has one) because they land in front of a user who asked to leave or refresh the coin (wavelength#577). Nothing legitimate re-issues the event: the manager dedups within a request, largest-first selection is `Live`-scoped, and the send-onchain intent replayer's restart-time re-reserve runs after `releaseOrphanedForfeits` has returned orphaned pending forfeits to `Live`.
