...
- `ReleaseSpendRequest` / `CompleteSpendRequest` — Spend lifecycle completion
  ...
  already spending. An absent entry (or nil map) releases unconditionally,
  preserving the behaviour for callers that hold no epoch (e.g. a manual
  unlock).
...
- `SelectedVTXO` — Describes a VTXO selected for spend (outpoint, amount,
  ...
  The epoch is echoed back on release so a superseded reservation can be
  refused; it is zero for reservations that are not spend reservations (e.g.
  forfeit inputs).
...
- **Spend reservations are epoch-fenced.** A holder that carries a
  `SelectedVTXO.ReserveEpoch` into its durable state must present that epoch on
  `ReleaseSpendRequest`. The manager releases only if the VTXO's current epoch
  still matches, so a redelivered release from a rolled-back session cannot
  unlock a coin a newer session has since reserved and is spending.
