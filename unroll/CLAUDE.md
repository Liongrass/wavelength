...
- **Checkpoint persists the sweep tx** via `wire.MsgTx.Serialize`
  ...
  Every checkpoint written by this build uses version 2, including exits
  that have not repriced. Version 1 remains readable; older binaries refuse
  version 2, so downgrade requires an explicit recovery plan for any exit
  checkpointed by this build.
