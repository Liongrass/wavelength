...
| `history_non_duplication` | `ListTransactionHistory` returns no two rows with the same `(txid, output_index, round_id, session_id, amount_sat, subtype)`. The round and session ids are in the key because round-level rows carry no chain identity — an empty `txid` and an `output_index` of `-1` — so two equal-amount sends of the same subtype in different rounds would otherwise collide into a false duplicate. A row with neither a chain identity nor a round or session id is skipped, since nothing distinguishes it from another such row. The history is read in a single 100,000-row page; when it fills, the check says in its `note` that only that prefix was examined. |
