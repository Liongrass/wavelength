...
| [custom-receive-registration.md](custom-receive-registration.md) | `RegisterPolicyReceiveScript`: registering an unfunded custom Taproot output for indexer queries — policy-committing ownership proof (TLV record 12), operator-side reconstruction, the upserted `(principal, pkScript)` binding, and 28-day bounded retention |
